...
def cholect45_ivtmetrics_mAP_all(df, CFG):
    ...
    read n_triple, map_file_path from CFG.
    (this function already includes 'iv' and 'it'; no change needed.)
    """
    n_triple = getattr(CFG, "n_triple", 170)
    map_path = getattr(CFG, "map_file_path", "maps.txt")
    ...
    for fold in tqdm(range(CFG.n_fold)):
        ...
        # [★ ★ ★ typo fix] '_df.video.unique()' -> 'vids = fold_df.video.unique()'
        vids = fold_df.video.unique()
...
def per_epoch_ivtmetrics_all(fold_df, CFG):
    ...
    read n_triple, map_file_path from CFG.
    Also computes iv_mAP, it_mAP and adds to return values (6 values total).
    """
    n_triple = getattr(CFG, "n_triple", 170)
    map_path = getattr(CFG, "map_file_path", "maps.txt")
    ...
    iv_mAP = rec.compute_video_AP("iv", ignore_null=CFG.ignore_null)["mAP"]
    it_mAP = rec.compute_video_AP("it", ignore_null=CFG.ignore_null)["mAP"]
    
    # 6 items value return
    return mAP, i_mAP, v_mAP, t_mAP, iv_mAP, it_mAP
#===============================================================================================
